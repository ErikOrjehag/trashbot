...
hold on
plot(MSE(1,:),'r')
legend({'Kalman filter calculate $P^{1,1}_k$','Simulated (true) $E[(x^1_k-\hat{x}^1_k)^2]$'},'Interpreter','latex','FontSize',12)
box on;
grid on;
...
hold on
plot(MSE(2,:),'r')
legend({'Kalman filter calculate $P^{2,2}_k$','Simulated (true) $E[(x^2_k-\hat{x}^2_k)^2]$'},'Interpreter','latex','FontSize',12)
box on;
grid on;

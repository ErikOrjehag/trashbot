...
Q=(0.1*T*T)^2;            % Process noise [(m/s^2)^2]
R=(1.0*T)^2;              % Measurement noise [m^2]

PI0=diag([R Q]);   % Initial state covariance
...
Q_kf=Q;            % Process noise [(m/s^2)^2]
R_kf=R;              % Measurement noise [m^2]

PI0_kf=diag([Q_kf Q_kf]);   % Initial state covariance
...
plot(t,x_hat(1,:),'r--')
plot(t,y(1,:),'b*')
legend({'$\pm 3\sqrt{P^{1,1}_k}$','$x^1_k$','$\hat{x}^1_k$','$y_k$'},'Interpreter','latex','FontSize',12)
box on;
grid on;
ylabel('Position [m]')
...
plot(t,x(2,:),'k')
plot(t,x_hat(2,:),'r--')
legend({'$\pm 3\sqrt{P^{2,2}_k}$','$x^2_k$','$\hat{x}^2_k$'},'Interpreter','latex','FontSize',12)
box on;
grid on;
xlabel('Time [s]')
ylabel('Speed [m/s]')
